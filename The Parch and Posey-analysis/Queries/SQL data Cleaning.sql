...
SELECT SUM(vowel) nums, SUM(non_vow) not_vowels
FROM (SELECT name, CASE WHEN LEFT(UPPER(name), 1) IN ('A', 'E', 'I', 'O', 'U') 
                       THEN 1 ELSE 0 END AS vowel, 
                   CASE WHEN LEFT(UPPER(name), 1) IN ('A', 'E', 'I', 'O', 'U') 
                       THEN 0 ELSE 1 END AS non_vow
      FROM accounts) t1;
...
select primary_poc, left(primary_poc, strpos( primary_poc, ' ') - 1) first_name, right (primary_poc, length(primary_poc) - strpos(primary_poc, ' ')) last_name
from accounts
...
/*
use COALESCE to fill in the accounts.id column with account.id for the NULL value .
*/
